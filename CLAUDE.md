...
### ADR-006: 提交后自动推送 (2026-05-11)

**决策**：`git commit` 后自动执行 `git push origin main`，除非用户显式禁止。

**原因**：
- 用户期望提交即同步到 GitHub，避免本地堆积多个未推送提交
- 减少"你代码上传到哪个地址了"这类澄清式对话
- 推送失败不阻塞（网络问题常见于国内访问 GitHub）
...
**核心规则：代码修改提交后自动推送至远程仓库。**

| 场景 | 行为 |
|------|------|
| 用户要求提交代码 | `git commit` → 紧接着 `git push origin main` |
| 推送失败（网络/权限） | 告知用户，提交保留本地，不阻塞后续工作 |
| 用户明确说"只提交不推送" | 尊重用户意愿，仅本地提交 |
| 实验性/破坏性修改 | 先 commit 不 push，等用户确认后再 push |

**判断逻辑**：
1. 默认：commit 后自动 push
2. 例外：用户显式说"不要推送"/"只保存在本地"
3. 推送失败不重试，不阻塞
